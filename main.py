...
game_data_table = {
    'team_id': 'integer','date': 'integer', 'opp_id': 'integer', 'loc': 'integer', 'home_away_neutral': 'varchar', 'result': 'varchar',
...
if response == 'Yes':
    ...
    for i, row in enumerate(game_pages.get_data(date(2014, 12, 4), date(2018, 10, 1)), start=1):
        if data_dict == {}:
